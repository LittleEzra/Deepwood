...
public class DeepwoodCreativeModeTabs {
    private static final DeferredRegister<CreativeModeTab> REGISTER =
            DeferredRegister.create(Registries.CREATIVE_MODE_TAB, Deepwood.MOD_ID);

    public static final DeferredHolder<CreativeModeTab, CreativeModeTab> BASE_CREATIVE_TAB = REGISTER.register("base",
            () -> CreativeModeTab.builder()
                    .title(Component.translatable("creative_tab.deepwood.base"))
                    .withTabsBefore(CreativeModeTabs.SPAWN_EGGS)
                    .icon(() -> new ItemStack(DeepwoodItems.SPECTERLITE.get()))
    ...
    private static class DisplayItemsGenerator implements CreativeModeTab.DisplayItemsGenerator {
        ...
        private List<Item> collectBlocks(Predicate<Item> exclusionPredicate) {
            ...
            for (DeferredHolder<Block, ?> entry : DeepwoodBlocks.BLOCKS.getEntries()) {
                Item item = entry.get()
                        .asItem();
                if (item == Items.AIR)
                    continue;
        ...
        private List<Item> collectItems(Predicate<Item> exclusionPredicate) {
            List<Item> items = new ReferenceArrayList<>();
            for (DeferredHolder<Item, ?> entry : DeepwoodItems.ITEMS.getEntries()) {
                Item item = entry.get();
